...
def approx_standard_normal_cdf(x):
    ...
    standard normal.
    """
    return 0.5 * (1.0 + th.tanh(th.sqrt(2.0 / th.tensor(np.pi, dtype=th.float32)) * (x + 0.044715 * th.pow(x, 3))))
...
class GaussianDiffusion:
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t, t,condition, clip_denoised=True, model_kwargs=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, t,condition, clip_denoised=clip_denoised, model_kwargs=model_kwargs
        )
        kl = normal_kl(
